...
export default function AdvancedFilters({ apps, users, features }: { apps: AppDto[]; users: UserDto[]; features: FeatureDto[] }) {
  ...
  const featureIdFilter = searchParams.get("featureId") || "";
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 relative">
      ...
      <div className="bg-neutral-900/60 backdrop-blur-md border border-neutral-800/50 rounded-2xl shadow-lg shadow-black/20 p-4 relative" style={{ zIndex: 1 }}>
        <label className="block text-sm font-medium text-neutral-200 mb-2">Özellik</label>
        <SearchableSelect
          options={features}
          value={featureIdFilter}
          onChange={(value) => updateFilter("featureId", value)}
          getLabel={(feature) => `${feature.name} (${feature.key})`}
          getValue={(feature) => feature.id}
          placeholder="Tüm Özellikler"
